...
function toRequestBody(input: GoogleEventInput) {
  ...
  return {
    ...
    ...(input.autoGenerateMeet ? { conferenceData: { createRequest: { requestId: randomUUID(), conferenceSolutionKey: { type: 'hangoutsMeet' } } } } : {}),
  }
}
...
function extractMeetLink(data: { hangoutLink?: string | null; conferenceData?: { entryPoints?: { entryPointType?: string | null; uri?: string | null }[] | null } | null }): string | null {
  return data.conferenceData?.entryPoints?.find((entry) => entry.entryPointType === 'video')?.uri ?? data.hangoutLink ?? null
}
...
export async function createGoogleEvent(refreshToken: string, input: GoogleEventInput): Promise<GoogleEventResult> {
  try {
    const res = await calendarClientFor(refreshToken).events.insert({
      calendarId: 'primary', sendUpdates: 'all', requestBody: toRequestBody(input),
...
export async function updateGoogleEvent(refreshToken: string, googleEventId: string, input: GoogleEventInput): Promise<GoogleEventResult> {
  try {
    ...
    const res = await calendarClientFor(refreshToken).events.patch({
      calendarId: 'primary', eventId: googleEventId, sendUpdates: 'all', requestBody: toRequestBody(input),
      conferenceDataVersion: input.autoGenerateMeet ? 1 : undefined,
    })
    return { id: res.data.id ?? googleEventId, meetLink: extractMeetLink(res.data) }
  } catch (error) {
