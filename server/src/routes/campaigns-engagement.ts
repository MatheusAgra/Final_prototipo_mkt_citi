...
campaignsRouter.post('/', asyncRoute(async (req, res) => { const body=campaignBody.parse(req.body); const {canais,metas,...data}=body; const row=await prisma.campaign.create({data:{...data,canais:{create:canais.map((canal)=>({canal}))},metas:{create:metas.map((m,i)=>({...m,ordem:i}))}},include:campaignInclude}); res.status(201).json(serializeCampaign(row)) }))
...
campaignsRouter.get('/:id/metrics',asyncRoute(async(req,res)=>res.json(await prisma.campaignDailyMetric.findMany({where:{campaignId:String(req.params.id)},orderBy:{data:'asc'},include:{valores:true}}))))
...
campaignsRouter.post('/:id/metrics',asyncRoute(async(req,res)=>{
  if(!await prisma.campaign.findUnique({where:{id:String(req.params.id)}}))throw new ApiError(404,'NOT_FOUND')
  const body=z.object({data:z.coerce.date(),alcance:z.number().int().min(0),interacoes:z.number().int().min(0),mostrarGrafico:z.boolean().default(true),valores:z.array(z.object({nome:z.string().trim().min(1),valor:z.number()})).optional().default([])}).parse(req.body)
  const metric=await prisma.$transaction(async(tx)=>{
    const row=await tx.campaignDailyMetric.upsert({where:{campaignId_data:{campaignId:String(req.params.id),data:body.data}},create:{campaignId:String(req.params.id),data:body.data,alcance:body.alcance,interacoes:body.interacoes,mostrarGrafico:body.mostrarGrafico},update:{alcance:body.alcance,interacoes:body.interacoes,mostrarGrafico:body.mostrarGrafico}})
    await tx.campaignMetricValue.deleteMany({where:{dailyMetricId:row.id}})
    if(body.valores.length)await tx.campaignMetricValue.createMany({data:body.valores.map((v)=>({dailyMetricId:row.id,nome:v.nome,valor:v.valor}))})
    return tx.campaignDailyMetric.findUniqueOrThrow({where:{id:row.id},include:{valores:true}})
  })
